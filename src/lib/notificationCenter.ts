...
// import { shouldWin10orGreater, shouldStringNotEmpty } from "@xan105/is/lib/assert";

async function remove(appID, uniqueID = null as any) {

  // shouldWin10orGreater(); // TODO
  ...
  if (uniqueID) {
    if (!Array.isArray(uniqueID)) {
      ...
      if (groupLabel && !tag) {
        ...
        Helpers.error('groupLabel can not be omitted')
      }
    } else {
      // throw new Failure("uniqueID as an array is [tag or null, groupLabel]","ERR_MALFORMED_ARGS");
      Helpers.error('uniqueID as an array is [tag or null, groupLabel')
    }
  }
  else {
    clear += `Clear('${appID}')`;
  ...
  if (ps.stderr) {
    // throw new Failure(ps.stderr, "ERR_UNEXPECTED_POWERSHELL_FAIL");
    Helpers.error("ERR_UNEXPECTED_POWERSHELL_FAIL")
  }
