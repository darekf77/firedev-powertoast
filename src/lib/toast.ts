...
const winRT = void 0; // = await loadWinRT();

async function notify(option = {} as IOption) {

  // shouldWin8orGreater();

  const legacy = false; // isWin8(); //Windows 8 && Windows 8.1
  const options = normalize(option, legacy) as any;
  // const powerShell = !winRT || (winRT && options.disableWinRT === true) ? true : false;
  const powerShell = true;
  const template = generate(options, powerShell, legacy);

  if (powerShell) {
    const rng = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min; // @ts-ignore
    ...
    try {
      ...
      Helpers.writeFile(scriptPath, template);
      ...
      if (options.callback && ps.stdout) {
        if (ps.stdout.includes("<@onFailed/>")) {
          Helpers.error('Failed to raise notification')
      ...
      Helpers.removeFileIfExists(scriptPath)
    } catch (err) {
      Helpers.removeFileIfExists(scriptPath);
      throw err;
    }
